	...
	if request.user.is_authenticated and request.user.is_staff:
		...
		if usu:
			try:
				...
				pres.save()
				info = "Guardó satisfactoriamente"
			except:
...
def eliminar_prestamo_view(request, id_prest):
	if request.user.is_authenticated and request.user.is_staff:
		...
		info = "El préstamo se eliminó safisfactoriamente"
	...
	if request.user.is_authenticated and request.user.is_staff:
		...
		if a == None:
			try:
				...
				info = "El autor que desea eliminar no nxiste"
				return render_to_response ('/autores')
		else:
				info = "El autor no se puede eliminar ya que existe algún libro que esté relacionado"
				return HttpResponseRedirect ('/autores')
	else:
		return HttpResponseRedirect ('/')
	...
	if request.user.is_authenticated and request.user.is_staff:
		...
		if request.method =="POST": #SI ES POST
			...
			if formulario.is_valid():
				...
				info = "Guardado satisfactoriamente"
				ctx = {'form':formulario, 'now':now, 'm':m}
				informacion = "Se Guardó Satisfactoriamente"
	...
	if request.user.is_authenticated and request.user.is_staff:
		...
		if e == None:
			...
			try:
				...
				info="La editorial no se puede eliminar ya que existe algún libro que esté relacionado"
				return HttpResponseRedirect('/editoriales/')
	else:
		return HttpResponseRedirect ('/')
	...
	if request.user.is_authenticated :
		...
		if request.method == "POST":
			if request.user.usuario.tipo_usuario.nombre == "bibliotecario":
				...
				mensaje =" Lo sentimos pero no puedes registrarte por que debe ser mayor de 18 años"
